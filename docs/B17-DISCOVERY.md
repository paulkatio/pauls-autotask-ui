...
- **Resend-Platzhalter ergänzt (2026-06-05):** `RESEND_API_KEY`, `RESEND_FROM`,
  `AUTOTASK_INBOUND_MAILBOX` (Reply-To) sind jetzt in `.env.example` + `.env.local`
  angelegt (leer; Werte = Paul-Fragen, siehe §5). **Resend = einziger Versandweg, kein
  `MAIL_PROVIDER`-Toggle (Paul, 2026-06-05).**
...
5. **Threading — GEKLÄRT (Paul, 2026-06-05):** Sobald die **Ticketnummer im Betreff**
   steht, threadet Autotask die Antwort zurück ans Ticket. Der historische
   `[Ticket#<16 Ziffern>]`-Token ist NICHT nötig. Offen nur noch: exakte
   Inbound-Mailboxadresse für `Reply-To` (Frage 4) — ein einmaliger Prod-Gegencheck genügt.
...
- **Betreff:** **Ticketnummer in den Betreff** — das genügt fürs Threading (Paul
  bestätigt: Ticketnummer im Betreff → Antwort landet wieder am Ticket). Format z. B.
  `[<ticketNumber>] <Titel>`. Der historische Autotask-Token `[Ticket#…]` wird NICHT
...
### 6.4 Kein Umschalter — Resend ist der einzige Versandweg (Paul, 2026-06-05)

Kein `MAIL_PROVIDER`-Flag, kein Autotask-Fallback: die Chat-Kundenmail läuft
**ausschließlich** über Resend. Der Toggle „Kunde benachrichtigen" in der Chat-UI
entfällt — eine Chat-Nachricht IST die Kundenmail (interne Vermerke laufen über die
separate interne Notiz, noteType 2). Konsequenz: beim Cutover muss die Autotask-
Workflow-Regel deaktiviert werden (Frage 7); ein Rückzug aufs alte Verhalten ginge nur
über Code/Git, nicht per Env-Schalter — bewusst akzeptiert.
...
1. **Threading — entschärft (Paul, 2026-06-05).** Autotask threadet Antworten über die
   **Ticketnummer im Betreff** zurück ans Ticket; der Autotask-eigene `[Ticket#…]`-Token
   ist nicht nötig. Das ursprüngliche „Threading-Bruch"-Hauptrisiko ist damit vom Tisch.
   Rest: einmaliger Prod-Gegencheck mit der echten Inbound-Mailbox als `Reply-To`.
