...
export default async function CompanyDetailPage({
  ...
  try {
    panel = await renderPanel(tab, companyId, sp.cursor, sp.q, picklists, {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        <StatCard
          ...
          icon={TicketIcon}
          href={`/companies/${companyId}?tab=offen`}
        />
        <StatCard
          title="Abgeschlossen"
          value={stats?.closedTickets ?? "—"}
          icon={CircleCheckIcon}
          href={`/companies/${companyId}?tab=abgeschlossen`}
        />
        <StatCard
          title="Kontakte"
          value={stats?.contacts ?? "—"}
          icon={UsersIcon}
          href={`/companies/${companyId}?tab=kontakte`}
        />
        <StatCard
          title="Geräte"
          value={stats?.devices ?? "—"}
          icon={MonitorIcon}
          href={`/companies/${companyId}?tab=geraete`}
        />
        <StatCard
          title="Verträge"
          value={stats?.contracts ?? "—"}
          icon={FileTextIcon}
          href={`/companies/${companyId}?tab=vertraege`}
        />
      </div>
